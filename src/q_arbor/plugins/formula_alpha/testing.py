"""Closed formula-alpha mock capability for C9 qualification."""
...
from . import FormulaAlphaPlugin, FormulaAlphaSplitData, FormulaMockOutcome
...
class _FormulaMockDevelopmentSplit:
    __slots__ = (
        "_artifacts",
        "_binding",
        "_contract",
    ...
    def __init__(self) -> None:  # pragma: no cover - construction is closed
        raise TypeError("use make_formula_alpha_mock_development_split")
    ...
    def _create(
        ...
        *,
        ...
        data: FormulaAlphaSplitData,
        artifacts: ArtifactSink,
    ) -> _FormulaMockDevelopmentSplit:
    ...
    def __setattr__(self, name: str, value: object) -> None:
        if getattr(self, "_initialized", False):
            raise AttributeError("authorized formula split is immutable")
    ...
    @property
    def data(self) -> FormulaAlphaSplitData:
        return self._data
    ...
    def _contaminated_result(self) -> EvaluationResult:
        # C5 G01/G02/G05 and C6 J01/J04: never retain suspect observations.
    ...
    def make_result(
        ...
        *,
        ...
        if status not in {"implementation_failure", "incomparable"}:
            raise EvaluationInvariantError("formula mock result status is unsupported")
...
def make_formula_alpha_mock_development_split(
    request: EvaluationRequest,
    contract: QuantResearchContract,
    candidate: ValidatedCandidate,
    plugin: FormulaAlphaPlugin,
    ...
    *,
    ...
    """Mint one of the two scoreless formula development outcomes."""

    if not isinstance(request, EvaluationRequest):
        raise EvaluationSchemaError("formula request type mismatch")
    if request.split_role != "development":
        raise EvaluationBoundaryError("formula mock factory is development-only")
    if (
        not isinstance(contract, QuantResearchContract)
        or not isinstance(candidate, ValidatedCandidate)
        or not isinstance(plugin, FormulaAlphaPlugin)
        or not isinstance(runtime_lock, VerifiedRuntimeLock)
        or not isinstance(artifact_store, ContentAddressedArtifactStore)
        or not isinstance(outcome, FormulaMockOutcome)
    ):
        raise EvaluationSchemaError("formula mock factory input type mismatch")
    mapping = contract.to_dict()
    development = mapping["data"]["splits"]["development"]
    if (
        mapping["task_kind"] != "formula_alpha"
        or mapping["data"]["schema_sha256"] != plugin.public_schema.sha256
    ):
        raise EvaluationIntegrityError("formula contract/schema identity mismatch")
    ...
    if (
        ...
        raise EvaluationIntegrityError("formula fold policy mismatch")
    ...
    return _FormulaMockDevelopmentSplit._create(
        request=request,
        contract=contract,
        binding=binding,
        data=data,
...
__all__ = ["make_formula_alpha_mock_development_split"]
